...
class LecturaTab extends ConsumerStatefulWidget {
  ...
  const LecturaTab({super.key, this.selectedNIS});
...
class _LecturaTabState extends ConsumerState<LecturaTab> {
  ...
  Future<void> _consultar() async {
    ...
    token = authState.value?.user?.token;

    final nisObtenido = widget.selectedNIS!.nisRad.toString();
    ...
    try {
      ...
      final consultaSituacionActualResponse = await repoConsultaSituacionActual
          .getMiCuentaSituacionActual(nisObtenido, token);
  ...
  Widget mostrarLecturaNOHabilitada() {
    return Padding(
      ...
      child: Column(
        children: [
          CustomText(
            "NIS: ${widget.selectedNIS?.nisRad.toString()} - ${situacionActualResultado!.nombre} ${situacionActualResultado!.apellido}",
            fontWeight: FontWeight.bold,
            fontSize: 18,
            overflow: TextOverflow.clip,
          ),
            ...
            child: situacionAsync.when(
              ...
              data: (situacion) => Column(
                ...
                children: [
                  ...
                  /*CustomCard(
                    child: Text(
                      situacion.habilitarAporteLectura?.mensaje ??
                          widget.selectedNIS?.nisRad.toString(),
                    ),
                  ),*/
